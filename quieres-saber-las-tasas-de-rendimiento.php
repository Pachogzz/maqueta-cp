<?php include('inc/header.php'); ?>

		<section class="after-flange header-title-inner bg-gradient-green">
			<div class="container h-100">
				<div class="row h-100 align-items-center">
					<div class="col">
						<h1 id="sectionTitle" class="display-4 br-30">¿Quieres saber las tasas de rendimiento?</h1>
					</div>
				</div>
			</div>
		</section> <!-- /. Page Title -->

		<?php include('inc/navs/inner_nav_page-quiero-ser-socio.php') ?>

		<section class="featured-module-home py-5">
			<div class="container">
				<div class="row">
					<div class="col-md-10 offset-md-1">
						<p class="h3 text-center w-100 font-weight-bold">Tasas de rendimiento</p>
						<p class="h4 text-center w-100 font-weight-medium">Caja Popular Mexicana, S.C. de A.P. de R.L. de C.V <br>Tasas brutas de interés aplicables a pagarés con rendimiento liquidable al vencimiento del 30 de Enero al 05 de Febrero del 2020.</p>
					</div>
				</div>
			</div>
		</section> <!-- /. Main Tag Line -->
		<section class="pb-5">
			<div class="container">
				<div class="row">
					<div class="col-12">
						<!-- Nota Legal Tabla -->
						<div class="table-responsive mt-5">
							<table class="table text-center">
								<thead>
									<tr class="bg-primary">
									<th scope="col"><span class="text-white">Rango Montos</span></th>
									<th scope="col"><span class="text-white">30 días</th>
									<th scope="col"><span class="text-white">60 días</th>
									<th scope="col"><span class="text-white">90 días</th>
									<th scope="col"><span class="text-white">120 días</th>
									<th scope="col"><span class="text-white">150 días</th>
									<th scope="col"><span class="text-white">180 días</th>
									<th scope="col"><span class="text-white">360 días</th>
								</tr>
								</thead>
								<tbody>
									<tr>
										<td scope="row">$ 1000.00 A $ 25,000.00</td>
										<td>3.84%</td>
										<td>4.05%</td>
										<td>4.25%</td>
										<td>4.46%</td>
										<td>4.67%</td>
										<td>4.89%</td>
										<td>6.20%</td>
									</tr>
									<tr>
										<td scope="row">$ 25,001.00 A $ 50,000.00</td>
										<td>4.01%</td>
										<td>4.22%</td>
										<td>4.43%</td>
										<td>4.64%</td>
										<td>4.86%</td>
										<td>5.07%</td>
										<td>6.39%</td>

									</tr>
									<tr>
										<td scope="row">$ 50,001.00 A $ 100,000.00</td>
										<td>4.18%</td>
										<td>4.38%</td>
										<td>4.59%</td>
										<td>4.81%</td>
										<td>5.02%</td>
										<td>5.24%</td>
										<td>6.56%</td>

									</tr>
									<tr>
										<td scope="row">$ 100,000.00 A $ 250,000.00</td>
										<td>4.32%</td>
										<td>4.53%</td>
										<td>4.75%</td>
										<td>4.96%</td>
										<td>5.18%</td>
										<td>5.39%</td>
										<td>6.72%</td>

									</tr>
									<tr>
										<td scope="row">$ 250,001.00 A $ 500,000.00</td>
										<td>4.47%</td>
										<td>4.68%</td>
										<td>4.89%</td>
										<td>5.10%</td>
										<td>5.32%</td>
										<td>5.53%</td>
										<td>6.86%</td>

									</tr>
									<tr>
										<td scope="row">$ 500,001.00 A $ 1,000,000.00</td>
										<td>4.60%</td>
										<td>4.81%</td>
										<td>5.02%</td>
										<td>5.24%</td>
										<td>5.44%</td>
										<td>5.66%</td>
										<td>7.00%</td>

									</tr>
									<tr>
										<td scope="row">$ 1,000,000.00 EN ADELANTE</td>
										<td>4.72%</td>
										<td>4.93%</td>
										<td>5.13%</td>
										<td>5.35%</td>
										<td>5.57%</td>
										<td>5.79%</td>
										<td>7.12%</td>

									</tr>
								</tbody>
							</table>
						</div>
						<!-- Nota Legal Tabla -->
						<p class="text-right"><small>* Las tasas de interés están expresadas en términos anuales</small></p>
					</div>
				</div>
			</div>
		</section> <!-- /. Main Content -->

<?php include('inc/footer.php'); ?>