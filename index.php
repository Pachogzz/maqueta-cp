<!-- <div class="alert alert-warning text-center mb-0" role="alert">
  Lamentamos mucho la molestia; presentamos de momento un pequeño fallo técnico <br>que estamos resolviendo a la brevedad, te invitamos a ingresar más tarde.
</div> -->
<?php include('inc/header.php'); ?>

<?php include('inc/home-slider-yt.php'); ?>

		<section id="inner-nav" class="inner-nav-home pb-3 pb-lg-5 bg-white">
			<div class="container">
				<div class="row text-center">
					<div class="col-md-12 mb-3 mb-lg-5">
						<h1 id="sectionTitle" class="d-none">Bienvenido a</h1>
						<h2 class="display-4 text-primary font-weight-semibold">¿En qué te podemos ayudar?</h2>
					</div>
					<div class="col-12 col-md-12 col-lg-4 mb-2 mb-lg-0 mb-xl-0">
						<a href="ser-socio" class="btn btn-primary btn-lg btn-block rounded-pill w-100 w-lg-75 mx-auto p-3">
							Quiero ser socio <i class="fa fa-chevron-circle-right"></i>
						</a>
					</div>
					<div class="col-12 col-md-12 col-lg-4 mb-2 mb-lg-0 mb-xl-0">
						<a href="necesitas-credito" class="btn btn-primary btn-lg btn-block rounded-pill w-100 w-lg-75 mx-auto p-3">
							Quiero un crédito <i class="fa fa-chevron-circle-right"></i>
						</a>
					</div>
					<div class="col-12 col-md-12 col-lg-4 mb-2 mb-lg-0 mb-xl-0">
						<a href="ahorro-e-inversion" class="btn btn-primary btn-lg btn-block rounded-pill w-100 w-lg-75 mx-auto p-3">
							Quiero ahorrar e invertir <i class="fa fa-chevron-circle-right"></i>
						</a>
					</div>
				</div>
			</div>
		</section> <!-- /. Inner Nav Home -->

		<section id="featured-module" class="featured-module-home">
			<div class="container">
				<div class="row">
					<div class="col-md-12 mb-3 mb-lg-5">
						<h2 class="display-3 text-center text-primary font-weight-semibold mb-3">¡Ya estamos más cerca de ti!</h2>
						<p class="display-5 mb-5 text-center">Conoce nuestro Servicio Electrónico por Internet, tus operaciones son + fáciles + seguras y ¡donde sea!</p>

						<div class="row">
							<div class="col-md-6 col-lg-7 offset-lg-1 mr-lg-n5 has-label">
								<img src="assets/img/secciones/inicio/0-banner-home-cpmmp.png" class="img-fluid" alt="CPM Móvil y CPM en Línea, ¡ahora con SPEI!">
							</div>
							<div class="col-md-6 col-lg-4 h-50 ml-lg-n5 my-auto">
								<div class="h-100 p-4 p-lg-5 bg-primary text-white br-ntl-30">
									<h3><strong>Aquí, nos seguimos renovando</strong></h3>
									<h5 class="text-yellow"><strong>Sácale más provecho a nuestro Servicio Electrónico por Internet</strong></h5>
									<p>Conoce CPM Móvil Plus, nuestro nuevo servicio con el que podrás obtener un Crédito Inmediato o contratar una Rendicuenta</p>
									<a href="cpm-movil-plus" class="btn btn-secondary br-30 px-5 font-weight-semibold">Saber más</a>
								</div>
							</div>
						</div>
					</div>
				</div>
			</div>
		</section> <!-- /. Featured Module Home -->

<?php include('inc/home-slider-second_.php') ?>

<?php include('inc/boletin-full-width.php') ?>

<?php include('inc/footer.php'); ?>