		<section class="inner-nav-page pb-3 pb-lg-5 bg-white d-none d-md-none d-lg-block">
			<div class="container">
				<div class="row text-center">
					<div class="col-12 col-lg z-index-1">
						<a href="#" class="btn btn-primary btn-lg btn-block rounded-pill w-100 w-lg-75 mx-auto p-3 mb-2 mb-lg-0 active">
							Queremos servirte <i class="fa fa-chevron-circle-right"></i>
						</a>
					</div>
					<div class="col-12 col-lg p-0 pl-md-0 pl-lg-3 mb-2 mb-lg-0 white-circle-n-line">
						<a href="crd20" class="btn bg-tertiary btn-lg btn-block w-100 rounded-0 py-3 px-0 text-white">
							Candidatos
						</a>
					</div>
					<div class="col-12 col-lg p-0 mb-2 mb-lg-0 ">
						<a href="candidatos-al-consejo-de-administracion" class="btn bg-primary btn-lg btn-block w-100 rounded-0 py-3 px-0 text-white">
							Consejo de Administración
						</a>
					</div>
					<div class="col-12 col-lg p-0 mb-2 mb-lg-0 ">
						<a href="candidatos-al-consejo-de-vigilancia" class="btn bg-tertiary btn-lg btn-block w-100 rounded-0 py-3 px-0 text-white">
							Consejo de Vigilancia
						</a>
					</div>
					<div class="col-12 col-lg p-0 mb-2 mb-lg-0 ">
						<a href="dirigentes" class="btn bg-primary btn-lg btn-block w-100 rounded-0 py-3 px-0 text-white">
							Dirigentes
						</a>
					</div>
				</div>
			</div>
		</section> <!-- /. Inner Nav Home Quiero ser Socio -->

		<section class="inner-nav-page in-nav-mobile d-block d-md-block d-lg-none">
			<div class="container">
				<div class="row text-center">
					<div class="col-12">
						<div class="dropdown">
							<a class="btn btn-primary rounded-pill text-white w-100 float-right" data-toggle="collapse" href="#innerMenuMobile" role="button" aria-expanded="false" aria-controls="innerMenuMobile">
								Queremos servirte <i class="fa fa-chevron-circle-down"></i>
							</a>
						</div>
						<div class="collapse w-100" id="innerMenuMobile">
							<ul class="list-group w-100 pt-3">
								<li class="list-group-item"><a href="crd20">Candidatos</a></li>
								<li class="list-group-item"><a href="candidatos-al-consejo-de-administracion">Candidatos al Consejo de Administrador</a></li>
								<li class="list-group-item"><a href="candidatos-al-consejo-de-vigilancia">Candidatos al Consejo de Vigilancia</a></li>
								<li class="list-group-item"><a href="dirigentes">Dirigentes</a></li>
							</ul>
						</div> <!-- ./ Menu Mobile Collapsable -->
					</div>
				</div>	
			</div>
		</section> <!-- /. Inner Nav Conocenos Mobile -->