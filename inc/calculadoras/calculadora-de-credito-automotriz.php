<a href="#" class="btn btn-primary btn-lg btn-block rounded-pill w-100 w-lg-75 mx-auto p-3 mb-3 mb-lg-5" data-toggle="modal" data-target="#calculadoraCredito">
	Calcular mi crédito <i class="fas fa-calculator"></i>
</a>
<!-- Modal -->
<div class="modal fade" id="calculadoraCredito" tabindex="-1" role="dialog" aria-labelledby="calculadoraCreditoTitle" aria-hidden="true">
	<div class="modal-dialog modal-dialog-centered modal-xl" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title" id="calculadoraCreditoTitle">Calcular mi crédito automotriz</h5>
				<button type="button" class="close" data-dismiss="modal" aria-label="Close">
					<span aria-hidden="true">&times;</span>
				</button>
			</div>
			<div class="modal-body">
				<iframe src="https://simuladorcredito.cpm.coop/SimuladorCreditosWeb/SimuladorWeb.aspx?IdProducto=417" frameborder="0" width="100%" height="500"></iframe>
				<!-- <iframe src="https://simuladorcredito.cpm.coop/SimuladorCreditosWeb/SimuladorWeb.aspx?IdProducto=414" frameborder="0" width="100%" height="500"></iframe> -->
			</div>
		</div>
	</div>
</div>